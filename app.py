...
if mode == "🏠 الصفحة الرئيسية":
    ...
    with col_bio:
        ...
        **شريكك الاستراتيجي في تحليل الأعمال والماليات**
    ...
    if uploaded_file:
        try:
            ...
            if not st.session_state.email_submitted:
                ...
                if len(num_cols) > 0:
                    if len(cat_cols) > 0:
                        st.plotly_chart(px.bar(df, x=cat_cols[0], y=num_cols[0], title="تحليل المبيعات حسب التصنيف"), use_container_width=True)
                    else:
