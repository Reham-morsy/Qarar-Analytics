...
""", unsafe_allow_html=True)
# --- كود تحسين المظهر وإضافة الفلاتر ---
st.markdown("""
    <style>
    /* تأثير الظل عند تمرير الماوس على البطاقات */
    div[style*="background-color: #1a3c34"]:hover {
        transform: translateY(-5px);
        transition: 0.3s;
        box-shadow: 0 10px 20px rgba(0,0,0,0.2);
    }
...
# --- إضافة الفلتر في القائمة الجانبية (Sidebar) ---
with st.sidebar:
    st.markdown("---")
    st.header("🔍 إعدادات الفلتر")
    selected_region = st.selectbox(
        "اختر المنطقة العقارية", 
